...
export const updateDemoInfo = async (
  ...
  // Use the ID from the token for security (prevents editing others)
  ...
  // Explicitly extract only the allowed demographic fields
  const { gender, dateOfBirth, location, occupation, relationship } = req.body;

  try {
    // findByIdAndUpdate respects your global pre(/^find/) middleware
