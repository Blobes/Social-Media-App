...
export const finalizeGistCreation = async (params: FinalizePostReq) => {
  return await finalizeGistCreation(params, {
    redisKey: FUNSTAKES_REDIS_URL,
    s3Config,
  });
};
...
export const finalizeGistUpdate = async (params: FinalizePostReq) => {
  return await finalizeGistUpdate(params, FUNSTAKES_REDIS_URL);
};
...
//   // Fetch the existing shell to read the user's initial hasSensitiveGraphic choice
//   const existingGistShell = await GistModel.findById(postId).session(session);

//   // Fallback cascade: Use true if either user marked it or AI detected it
...
//       },
//       topics: modResult.extractedTopics,
//       status: modResult.status,
//       hasSensitiveGraphic: determinedSensitiveGraphic,
//     },
...
//   // Fallback cascade for updates: Preserve existing document state if true, else fall back to incoming assessment
//   const determinedSensitiveGraphic =
