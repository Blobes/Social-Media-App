...
> {
  ...
  if (
    mode === "SINGLE" &&
    flags?.includeLanguage &&
    typeof rawResult === "object" &&
    rawResult !== null
  ) {
    const userId = (rawResult as { _id?: mongoose.Types.ObjectId })._id;

    if (userId) {
      const additions = await UserHydrationService.enrichLanguage(userId);

      (rawResult as Record<string, unknown>).additions = {
        ...((rawResult as Record<string, unknown>).additions as Record<
          string,
          unknown
        >),
        ...additions,
  ...
  if (isLean && typeof rawResult === "object") {
    if (!includeSensitiveFields) {
      rawResult = sanitizeUserResult(rawResult, userSensitiveFields());
    }
    if (!includePrivateFields) {
      rawResult = sanitizeUserResult(rawResult, userPrivateFields());
    }
  }
